...
export default function Lead({setload}) {
  ...
  const columns =  [
    ...
    { key: "lead_origin", title: "Origin", dataIndex: "origin", width: 160 },
    {
      ...
      render: (_, record) => {
        return <LeadTaskRelation selectLead={setSelectedLeads} leadRecord={setselectedLeadRecord} setredistribute={setredistribute}  record={record} assigned={record.assigned} openModal={()=>setOpenDistribute(true)} />;
      },
    },
  ...
  return (
    <>
      {contextHolder}
      {OpenDistribute&&<DistributeModal
      setload={setload}
      open={OpenDistribute}
      ...
      <div className="h-calc-remaining flex flex-col justify-between ">
        <div className="h-[80%]">
          <Table locale={{ emptyText: 'No Leads available' }}
            style={{ maxWidth: "100%" }}
