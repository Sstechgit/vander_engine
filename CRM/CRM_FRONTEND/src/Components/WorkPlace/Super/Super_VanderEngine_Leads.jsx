...
export default function Super_VanderEngine_Leads({ setload }) {
  ...
  const fetchLeads = async (page, pageRows) => {
    ...
    if (result.success) {
      ...
      let filteredLeads = records.filter(lead => lead?.origin?.toLowerCase() === "Vander Engines".toLocaleLowerCase());
