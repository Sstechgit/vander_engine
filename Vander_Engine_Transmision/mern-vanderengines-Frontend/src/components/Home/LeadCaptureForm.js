...
function LeadCaptureForm() {
  ...
  const handleChange = (e) => {
    ...
    setFormData({
      ...formData,
      [name]: value,
      ...(name === "carBrand" ? { carModel: "" } : {}), // Reset model when brand changes
    });
  };
  const availableModels = formData.carBrand ? makeModelMapping[formData.carBrand] : [];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('https://backend.vanderenginestransmissions.com/api/appointments', {
        method: 'POST',
        headers: {
  ...
  return (
    ...
    {isLoading && <LoadingSpinner />} {/* Display the loading spinner when loading */}
      <section className="lead-capture-form">
        <Container fluid>
          ...
          <Form onSubmit={handleSubmit}>
            ...
            <Form.Group controlId="formFullName">
              ...
              <Col md={6} sm={6} xs={6}>
                ...
                <Form.Group controlId="formPartNeeded">
                  <Form.Label>Part Needed:</Form.Label>
                  <Form.Select 
                    name="partNeeded" 
                    value={formData.partNeeded}
                    onChange={handleChange}
                    required
                    className="form-control" // Apply the same class as inputs
                  >
  ...
  >
    ...
    {Object.keys(makeModelMapping).map((brand, index) => (
      <option key={index} value={brand}>
        {brand}
              ...
              <Form.Group controlId="formModelYear">
